...
struct DocumentEditorView: View {
    ...
    @State private var revisionCounter: Int = 0
    ...
    var body: some View {
        ...
        .onChange(of: document.svgSource) { _, _ in reparse() }
        .onChange(of: documentURL?.path ?? "") { _, _ in reparse() }
    ...
    @ViewBuilder
    private func sidebar(_ parsed: SVGDocument) -> some View {
        VStack(spacing: 0) {
            ScrollView {
            ...
            .frame(maxHeight: useCanvasMode ? 200 : .infinity)
    ...
    private var toolPicker: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(activeTool == .select
                 ? "Click to select. Drag to pan. Pinch to zoom."
    ...
    // MARK: - Read-Only Inspector (Preview Mode)

    @ViewBuilder
    private func readOnlyInspector(_ element: SVGElement) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let fill = element.style.fill { summaryRow("fill", value: fill) }
            if let stroke = element.style.stroke { summaryRow("stroke", value: stroke) }
            if let strokeWidth = element.style.strokeWidth { summaryRow("stroke-width", value: formatNumber(strokeWidth)) }
            if let opacity = element.style.opacity { summaryRow("opacity", value: formatNumber(opacity)) }

            if element.transform != .identity {
                Divider()
                Text("Transform").font(.subheadline).foregroundStyle(.secondary)
                summaryRow("translate", value: "\(formatNumber(element.transform.tx)), \(formatNumber(element.transform.ty))")
                summaryRow("scale", value: "\(formatNumber(element.transform.a)), \(formatNumber(element.transform.d))")
            }
        }
    }
    ...
    private func formatOptionalSize(width: CGFloat?, height: CGFloat?) -> String {
        switch (width, height) {
        case let (width?, height?): return "\(formatNumber(width)) × \(formatNumber(height))"
        case let (width?, nil): return "\(formatNumber(width)) × —"
        case let (nil, height?): return "— × \(formatNumber(height))"
        default: return "—"
        }
    }
